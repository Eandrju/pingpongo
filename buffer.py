...
def magicPerspectiveProjector(points,
                              distanceFromPlane=PLANE_POSITION * PERSPECTIVE_PARAMETER):  # points -> ndarray with shape [x, 3]
    try:
...
class Obstacle(QtWidgets.QGraphicsItem):
    ...
    def paint(self, qp, o, widgets=None):
        ...
        for cubeSide in sorted(cubeSides, key=lambda x: np.linalg.norm(x.midPoint - np.array([o[0], o[1], 0])),
                               reverse=True):
            cubeSide.draw(qp)
...
class Racket(QtWidgets.QGraphicsItem):
    ...
    def paint(self, qp, o, widgets=None):
        ...
        qp.drawRect(self.origin[0] + self.projectedNodes[0][0], self.origin[1] + self.projectedNodes[0][1],
                    abs(self.projectedNodes[0][0] - self.projectedNodes[2][0]),
                    abs(self.projectedNodes[0][1] - self.projectedNodes[2][1]))

        qp.drawLine(self.origin[0] + self.projectedNodes[0][0],
                    self.origin[1] + (self.projectedNodes[0][1] + self.projectedNodes[2][1]) / 2,
                    self.origin[0] + self.projectedNodes[2][0],
                    self.origin[1] + (self.projectedNodes[0][1] + self.projectedNodes[2][1]) / 2)

        qp.drawLine(self.origin[0] + (self.projectedNodes[0][0] + self.projectedNodes[2][0]) / 2,
                    self.origin[1] + self.projectedNodes[0][1],
                    self.origin[0] + (self.projectedNodes[0][0] + self.projectedNodes[2][0]) / 2,
                    self.origin[1] + self.projectedNodes[1][1])
...
class Ball(QtWidgets.QGraphicsItem):
    ...
    def move(self):
        ...
        self.velocityVector += np.array(
            [-self.rotationVector[1], self.rotationVector[0], 0]) / BALL_SPIN_PARAMETER * BALL_SPIN_PARAMETER_DOS
        self.rotateSphere()
        self.setZValue(1 / self.position[2])
...
class Scene(QtWidgets.QGraphicsScene):
    def __init__(self, size, view):
        ...
        self.obstacle = Obstacle(self.windowSize, [-200, 200, PERSPECTIVE_PARAMETER * (
        START_POSITION + (END_POSITION - START_POSITION) / 2)], self, QtCore.Qt.gray, 100)
        ...
        self.scoreText = TextItem("FUCK: {0} : {1}".format(self.view.score[0], self.view.score[1]),
    ...
    def mouseMoveEvent(self, event):
        if self.moveracket:
            self.myRacket.move(np.array([event.scenePos().x() - self.origin[0], event.scenePos().y() - self.origin[1],
                                         self.myRacket.position[2]]))
    ...
    def checkCollision(self):
        ...
        if ballZ < start:
            ...
            if ballX > rect[0] and ballX < rect[0] + rect[2] and ballY > rect[1] and ballY < rect[1] + rect[3]:
                ...
                self.ball.rotationVector += (
                np.array([-self.myRacket.velocity[1], self.myRacket.velocity[0], 0]) / BALL_SPIN_PARAMETER_DOS)
            ...
            if ballX > rect[0] and ballX < rect[0] + rect[2] and ballY > rect[1] and ballY < rect[1] + rect[3]:
                ...
                self.ball.velocityVector[2] *= -1
                self.ball.rotationVector += (
                np.array([-self.enemyRacket.velocity[1], self.enemyRacket.velocity[0], 0]) / BALL_SPIN_PARAMETER_DOS)
                print(np.linalg.norm(self.ball.rotationVector))
            else:
                self.ball.velocityVector[2] *= -1
